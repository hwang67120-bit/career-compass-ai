...
상태: **폐기 — 2026-08-03 MVP에서 PDF·이력서·포트폴리오 입력 제거**

Java 구현은 제거됐다. Python 구현과 테스트는 Python 담당이 별도 정리할 때까지 남아 있으며, 이 문서는 이전 계약 이력 확인용으로만 유지한다.
...
이메일, 전화번호, 주민등록번호는 개인정보 제거 단계에서 정규식으로 제거하며(7절), `piiRemoved=true`는 이 항목들이 제거됐음을 보장한다.

이름, 생년월일, 사진과 상세 주소는 후보나 근거에 포함하지 않도록 LLM에 지시하지만, 정규식 등으로 기계적으로 강제하지는 않는다 — 정답 이름을 Java가 별도로 전달하지 않고(2026-07-29 자체 검증), Codex·Claude·Gemini 어느 쪽도 실명과 가명을 구분하는 방식을 아직 확보하지 못했다(2026-07-30 합의). 이름 등을 확실히 제거하려면 별도 학습(NER 등)이 필요하며, 지금은 `piiRemoved`의 보장 범위에 포함하지 않는다. `sourceText`에도 분석에 필요하지 않은 개인정보를 남기지 않는다.
