...
상태: **부분 확정 — API 경계·공공취업정보 Provider·공공기관 필터는 구현 기준, 검색어와 제한 수치는 확인 필요**
...
### 실 API 확인 결과 (2026-08-11)

- Decoding 인증키를 엄격하게 URL 인코딩한 `getList` 요청은 `resultCode=00`을 반환했다.
- 요청 필터 `Pblanc_ty=e08`, `Instt_se=g03`에 대해 응답 항목은
  `type01=e08`, `type02=g03`으로 확인됐다.
- Spring `queryParam`에 인증키를 값으로 직접 넣으면 Decoding 키의 `+`가 보존돼
  인증에 실패하므로, URI 템플릿 변수로 전달해 `%2B`로 인코딩한다.
- 동일 `idx`의 `getItem` 요청도 `resultCode=00`과 본문을 반환했다.
- 목록과 상세 응답의 `type01`/`type02` 값 체계는 동일하지 않았다. 공공기관 필터는
  목록 선택 단계에서만 적용하고 상세 단계는 선택된 `idx` 일치와 본문 존재를 검증한다.
- 키워드 검색 정확도는 별도 표본 검증이 필요하다.
