...
우선순위 4는 아래 "채용공고 외부 조회"의 Java API가 끝나야 실제 요청으로 검증할 수 있다.
그 전까지 Python은 임시 샘플 채용공고 텍스트로 임베딩·유사도·재정렬을 연결해 두고,
모델 정확도(어떤 모델·프롬프트가 근거 있는 결과를 내는지)를 계속 검증한다.
이 임시 연결은 `제안` 상태이며 실제 Java 요청으로 계약 검증을 마치기 전까지 완료로 보지 않는다.
...
- Java가 조회한 본문 텍스트는 기존 계약(`contracts/job-posting-extraction.md`)의 `sourceText`로 그대로
  전달하므로 Java–Python 계약을 새로 만들 필요는 없다.

| 우선순위 | 작업 | 계약 상태 | 시작 조건 |
| ---: | --- | --- | --- |
| 1 | 채용공고 URL 입력 → 허용 도메인 검증 → 서버사이드 본문 조회 API | 확인 필요 | 허용 도메인 최종 목록 확정, 각 사이트 약관상 서버 조회 허용 여부 확인 |
