...
## 채용공고 의미 비교 — 임베딩 호출 제외, 분석 모델 집중 (2026-08-11)

사용자 결정에 따라 MVP 분석 경로에서 임베딩 생성과 코사인 유사도 계산에 토큰·실행 자원을
사용하지 않는다. Gemini API를 제거하는 결정은 아니다. Ollama와 Gemini는 채용공고 추출과
근거 기반 분석에 집중하며, Ollama를 기본 Provider로 사용하고 Gemini는 실패 시 폴백과
제한적인 교차검증에 사용한다.
