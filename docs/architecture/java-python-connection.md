...
**Java 쪽에 필요한 구현 (아직 안 됨):**
1. Java도 같은 값을 환경변수(예: `INTERNAL_SERVICE_TOKEN`)로 읽는다.
2. `PythonHealthClient`(및 앞으로 추가될 모든 Python 호출 클라이언트)가 요청을 보낼 때마다 `X-Internal-Token` 헤더에 이 값을 실어 보낸다.
3. 두 서버의 `INTERNAL_SERVICE_TOKEN` 값은 반드시 동일해야 한다 (배포 시 같은 값을 양쪽에 주입).
