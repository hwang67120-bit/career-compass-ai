...
서버가 허용한 공식 채용 API 또는 명시적으로 활성화한 합성 공고에서 공고를 찾고 비교한다.
...
| 영역 | 책임 |
|---|---|
| Java | 인증·소유권, 프로필·저장소 선택, 공식 채용 API 또는 합성 공고 선택, 작업 상태·결과 저장, 명확한 조건 판정, 취소 |
...
- 합성 공고 공급자는 `demo` 프로필과
  `job-search.provider=dev-sample` 설정이 함께 있을 때만 활성화한다.
- Java는 분석에 고정된 프로필의 `targetJobTitle`을 합성 공고 제목과 정확히 비교한다.
- 일치한 공고만 요청 상한까지 반환하며 검색어를 무시하거나 백엔드 공고를 기본값으로
  반환하지 않는다.
- 일치 공고도 공식 공고와 같은 개인정보 제거, 구조화 추출, 사용자 확인, 의미 비교와
  결과 저장 흐름을 거친다.
- 합성 공고의 회사명과 출처는 가상 자료임을 브라우저에 표시하고 시장 통계에 사용하지 않는다.
- 일치 공고가 없으면 분석을 빈 `COMPLETED`로 끝내고 브라우저는
  `검색된 합성 공고 없음`을 표시한다.
- v1에서 제외한 `LLM 엔지니어`, 프롬프트 엔지니어와 AI 에이전트 엔지니어는
  다른 직무로 바꾸지 않고 검색 결과 0건으로 처리한다.

세부 데이터와 표시 규칙은
[합성 개발 직군 채용공고 Fixture](../testing/synthetic-job-posting-fixtures.md)를 따른다.
...
- 공고 담당 업무는 있지만 확정된 사용자 프로젝트 담당 업무가 없으면 비교 모델을 호출하지
  않고 `USER_EVIDENCE_EMPTY_AFTER_SANITIZATION`과 `NOT_CALCULABLE`을 저장한다.
- 공고 담당 업무 근거가 없으면 `JOB_EVIDENCE_EMPTY_AFTER_SANITIZATION`과
  `NOT_CALCULABLE`을 저장한다. 두 경우 모두 시스템 장애나 0점으로 표시하지 않는다.
