...
- 의미 비교의 `jobEvidenceId`는 `evidence`에 있는 실제 공고 근거 문장과 연결해 표시한다.
- `bestMatchUserEvidenceId`가 있으면 `evidence`에 있는 실제 사용자 프로젝트 근거 문장과
  연결해 표시한다. `공고 담당업무 1`처럼 순번만 있는 임시 문구는 사용하지 않는다.
- `NOT_CALCULABLE`에는 계산된 것처럼 보이는 막대를 표시하지 않고, 공고 근거 부족과
  사용자 프로젝트 근거 부족을 서로 다른 안내 문구로 표시한다.
...
1. `demo` 프로필의 합성 공고 공급자는 사용자의 희망 직무명과 정확히 일치하는 공고를 반환한다.
2. 합성 공고도 공식 공고와 같은 추출, 근거 검증과 저장 흐름을 사용한다.
3. 사용자가 프로젝트 담당 업무 후보를 확인한다.
4. Python이 공고 담당 업무와 확인된 프로젝트 담당 업무를 비교한다.
5. Java가 비교 응답과 근거 참조를 검증한 뒤 결과를 저장한다.
6. 브라우저는 실제 공고 근거 문장, 가장 관련 있는 사용자 프로젝트 근거 문장과
   `관련 있음` 또는 `관련 없음` 판정을 함께 표시한다.

합성 공고는 반복 검증용 가상 데이터임을 표시하며 시장 통계와 실제 채용 정보에는 포함하지 않는다.
...
공식 Provider(공급자) 또는 합성 공고 공급자가 정상적으로 0건을 반환하면 분석은 빈
`COMPLETED`가 된다. 결과 API는 `postings: []`를 반환하며 브라우저는 0% 차트 대신
`검색된 공고 없음`을 표시한다. 합성 공고 공급자는 일치하지 않는 직무를 다른 공고로
대체하지 않는다.
...
18. 의미 비교의 공고·프로젝트 근거 식별자가 `evidence`의 실제 문장과 연결됨
19. 합성 공고 검색 결과 없음이 다른 직무의 기본 공고로 대체되지 않음
