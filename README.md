...
**LLM을 신뢰할 수 있는 백엔드 서비스로 만드는 데 집중한 프로젝트입니다.** LLM 호출은
쉬워졌지만, 값은 "되게 하는 것"이 아니라 "믿게 하는 것"에 있다고 보고 다음에 집중했습니다.

- **근거 기반·무할루시네이션**: 모든 추출·판정을 원문 근거 식별자에 연결하고, 근거 없는
  값은 만들지 않습니다(`validate_evidence`).
- **측정으로 기술 선택**: 임베딩이 직무를 구분 못 함을 실측해 기각하고, 동일 fixture 품질
  게이트로 LLM-as-judge(`qwen2.5`)를 **best-match 36/36**으로 선정했습니다.
- **결정론 + 의미 분석 분리**: Java가 규칙 판정, Python이 근거 기반 의미 비교. 하나의
  불투명한 총점으로 합치지 않습니다.
- **실서버 검증·관측성**: 컨테이너 배포로 실연결하며 통합 버그 5건을 잡았고, LLM 호출당
  토큰·지연·에러율을 계측·집계합니다(`GET /internal/v1/metrics`).

> 기술 스택: Java 21 · Spring Boot · Python · FastAPI · Ollama/Gemini · PostgreSQL · Docker
...
서버 실배포에서 **로그인 → 공고 검색 → 개인정보 제거 원문 전달 → Python 구조화 추출**까지
브라우저로 검증했고, **공고 담당 업무 ↔ 프로젝트 업무 의미 비교(judge) 배선**과 비교 결과
저장·상태 전이까지 백엔드에 구현했습니다. 남은 마지막 조각은 **담당 업무 확정 UI와 결과
차트(프론트)**이며, 이것이 붙으면 로그인부터 결과까지 전체 흐름이 완성됩니다.

각 단계는 상태로 구분해, 추출 성공을 최종 분석 성공으로 뭉뚱그리지 않습니다(추출 완료와
비교·결과 완료를 별도 상태로 관리).
...
`nomic-embed-text` 임베딩은 관련 없는 문장에도 거의 같은 최고 점수를 줘 직무를 구분하지
못했습니다. 그래서 임베딩을 채택하지 않고, 같은 fixture로 LLM-as-judge를 평가해 `qwen2.5`를
선정했습니다(best-match 36/36으로 품질 게이트 통과). Gemini는 폴백·교차검증용입니다.
...
- GitHub OAuth와 사용자별 데이터 격리, 저장소 조회 인증(레이트리밋 대응)
- 목표 직무·기술 태그 프로필과 공개 GitHub 저장소 등록
- 공공취업정보 목록·상세 XML, 공공기관 필터와 개발 직군 검색
- Java Worker의 공고 정제·Python 호출
- Ollama 기반 공고 구조화 추출과 근거·응답 계약 검증, 추출 결과·모델 실행 정보 저장
- 임베딩·LLM-as-judge 동일 fixture 품질 평가(임베딩 기각, `qwen2.5` 게이트 통과)
- **공고 업무 ↔ 프로젝트 업무 의미 비교(judge) 배선, 비교 결과 저장과
  `COMPLETED / PARTIALLY_COMPLETED` 상태 전이**
- **LLM 호출 관측성**: 호출당 토큰·지연·성공여부 계측과 provider·stage별 집계 조회
- **컨테이너 배포로 서버 실연결** + 브라우저 로그인·검색·추출 검증(통합 버그 5건 수정)
- 요청 ID 기반 Java–Python 로그 추적

### 남은 작업 (대부분 프론트)

- 담당 업무 후보 **확정 UI**와 분석 **결과 차트** 렌더
- 확정 → 비교 → 결과까지 **전체 브라우저 흐름 검증**

핵심 백엔드 파이프라인(추출·비교·상태·관측성)은 구현됐고, 남은 조각은 사용자 확정
화면과 결과 시각화입니다. 상세 검증 상태는 [현재 작업 상태](docs/current-work.md)에서
관리합니다.
...
| 임베딩 점수가 업무 차이를 구분하지 못함 | 임베딩을 채택하지 않고, 동일 fixture 품질 게이트로 LLM-as-judge(`qwen2.5`) 선정 |
...
- 임베딩 유사도가 직무를 구분하지 못함을 측정으로 확인하고, LLM-as-judge로 전환해 품질 게이트로 검증한 과정
- 외부 데이터와 LLM 응답을 계약과 근거로 검증하는 구조
- Java의 결정론적 판정과 Python의 의미 분석을 분리한 설계
- 개인정보와 모델 장애를 고려한 서버 간 데이터 경계
- LLM 호출을 계측·집계해 신뢰성을 측정 가능하게 만든 관측성 설계
- 컨테이너 배포로 실연결하며 통합 버그를 잡아낸 과정
- 단위·연결·브라우저·배포 테스트를 구분한 완료 기준

핵심 백엔드 파이프라인(추출·의미 비교·상태·관측성)은 구현됐고, 사용자 확정 화면과
결과 시각화(프론트)가 붙으면 전체 흐름이 완성됩니다.
