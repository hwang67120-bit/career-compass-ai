...
아래 수치는 **2026년 8월 25일 브라우저 검증 1회**에서 확인한 결과입니다.
전체 채용시장이나 모델의 일반적인 정확도를 뜻하지 않습니다.

| 검증 대상 | 입력 규모 | 확인 결과 | 해석 |
|---|---:|---:|---|
| 공공취업정보 공고 | 조회 공고 5건 | 분석 가능한 담당 업무 0건 | 실제 API 연결은 성공했지만 개발 직무 분석 자료로는 부족 |
| 합성 공고 전체 흐름 | 공고 1건, 담당 업무 2건 | 추출·저장·화면 표시 2건 완료 | Java–Python 연결과 결과 표시 확인 |
| 합성 공고 의미 비교 | 담당 업무 2건 | 관련 있음 0건, 관련 없음 2건 | 현재 비교 모델의 품질은 확보하지 못함 |
| Java 자동 테스트 | 169건 | 통과 169건, 실패·오류·비활성 0건 | 백엔드 회귀 검증 |

합성 공고는 샘플로 표시하며 실제 채용시장 통계로 사용하지 않습니다.
공공 API 연결 성공이 곧 분석 가능한 데이터 확보를 뜻하지는 않았습니다.
합성 공고에서 전체 분석 흐름은 실행됐지만, 관련 있음 판정은 확인하지 못했습니다.
따라서 현재 결과로 의미 비교 정확도나 취업 적합도 품질을 주장하지 않습니다.

정확도를 수치로 제시하려면 관련 있음, 관련 없음과 근거 부족 사례를 사람이 먼저 판정한
별도 평가 자료가 필요합니다. 이 평가는 아직 수행하지 않았습니다.
...
`nomic-embed-text`는 제한된 예시에서 관련 없는 문장에도 높은 점수를 주어 채택하지 않았습니다.
같은 예시에서 `qwen2.5`가 기대한 판정을 반환해 비교 모델 후보로 선택했습니다.
다만 실제 전체 흐름에서는 합성 공고 담당 업무 2건을 모두 관련 없음으로 판단했으므로,
서비스 수준의 비교 품질이 확보됐다고 보지 않습니다.
당시 평가 범위는 [의미 비교 방식](docs/architecture/job-fit-semantic-similarity.md)에 정리했습니다.
...
브라우저 전체 흐름과 결과 표시는 확인했습니다.
다만 의미 비교 품질을 수치로 제시하려면 사람이 정답을 정한 평가 자료를 만들고
`RELATED / NOT_RELATED / NOT_CALCULABLE` 판정 일치율을 별도로 측정해야 합니다.
...
| 임베딩이 업무 차이를 구분하지 못함 | 제한된 예시에서 `qwen2.5`를 후보로 선택했지만 실제 비교 품질은 미확보 |
...
- 제한된 예시와 실제 전체 흐름의 결과가 다를 수 있음을 확인했습니다.
...
의미 비교 모델의 정확도는 아직 입증하지 못했으며, 현재 수치를 합격 가능성이나
취업 적합도 품질로 사용하지 않습니다.
