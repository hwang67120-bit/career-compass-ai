...
- **Gemini 폴백(2026-08-04)**: Ollama가 근거 검증 재시도까지 실패하면 Gemini로 폴백한다 —
  채용공고는 공개 정보라 이력서·희망 직무와 달리 Gemini 무료 등급 데이터 제한 정책 확인
  없이 실사용 폴백으로 쓴다. 실패하면 응답의 `modelProvider`가 `"gemini"`로 정직하게
  표시된다(`docs/current-work.md` 참고).
...
| `GEMINI_API_KEY`, `GEMINI_MODEL` | Gemini 연동. `GEMINI_MODEL`은 이제 채용공고 추출의 Ollama 폴백으로도 실사용된다(2026-08-04) | 확인 필요 |
